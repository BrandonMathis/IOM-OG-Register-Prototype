...
class NetworkTest < ActiveSupport::TestCase
  ...
  should "support an entry point as a network connection" do
    assert_valid Factory.create(:network, :entry_edges => [Factory.create(:network_connection)])
  end

  context "generating xml" do
    setup do
      @network = Factory.create(:network, 
                                :entry_edges => [Factory.create(:network_connection)])
    ...
    should "have proper xml name for each entry point" do
      assert_equal 1, @doc.xpath("//Network/EntryEdge").size
    end
  end
end
