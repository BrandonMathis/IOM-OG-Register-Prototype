...
class ObjectDatumTest < ActiveSupport::TestCase
  ...
  context "generating xml" do
    ...
    should "have proper path for data value" do
      assert_not_nil data_node = @doc.mimosa_xpath("//Entity[@*='Attribute']/Value/Text").first
      assert_equal @datum.data, data_node.content
    end

    should "have the proper path for attribute type" do
      assert_not_nil attribute_type_node = @doc.mimosa_xpath("//Entity[@*='Attribute']/Type").first
    end

    should "have the proper path for eng unit type" do
      assert_not_nil attribute_type_node = @doc.mimosa_xpath("//Entity[@*='Attribute']/UnitType").first
    end
  end
end
